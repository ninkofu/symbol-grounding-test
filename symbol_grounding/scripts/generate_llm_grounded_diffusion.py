"""CLI for LLM-driven grounded diffusion."""
...
from ..llm_scene_graph import LLMPlannerConfig
from ..pipeline_llm_grounded_diffusion import generate_llm_grounded_image


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate a grounded diffusion image via LLM planning.")
    ...
    parser.add_argument("--llm-api-base", default="https://api.openai.com/v1/chat/completions", help="LLM API base URL")
    return parser
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    llm_config = LLMPlannerConfig(model=args.llm_model, api_base=args.llm_api_base)

    try:
        result = generate_llm_grounded_image(
            prompt=args.prompt,
